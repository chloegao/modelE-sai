...
!@when using mass based aerosols
      ...
      real*8 AIRM,EXPL,EXPO,WCDNO,WCDNL,rho
     *,MCDNL1,MCDNO1,amass,tams,smturb,PL,TL
      real*8 SSM1,SSM2,SSM3,SSM4,SSM5,SSM6,SSM7,SSM8,SSM9,SSM10,
     *       SSMAL,SSMAO,SSML,SSMO
...
#endif  /* TRACERS_AEROSOLS_OCEAN */
#ifdef TRACERS_BrC
      SSM10= 1.70d12*(DSU(32)*fq_aer(n_BrC_w)
     &               +DSU(33)*fq_aer(n_BrC_b)
     &               +DSU(34)*fq_aer(n_BrC_t))
#else
      SSM10=0.d0
#endif  /* TRACERS_BrC */
c
...
!@input is mostly aerosol mass and a few cloud properties 
     ...
     *,FCLD,LHX
      ...
      real*8 SSMD1,SSMD2,SSMD3,SSM1a,SSM10
