...
c     LWaerosolCalcs=0: the spectra were precalculated above at Reff_mode.
      ...
      if ( LWaerosolCalcs==1 ) then
          ...
          DO n = 1,nmodes
             ...
             ENDDO   ! dry species
c SAI: calcite (dry_Vf_LEV slot 8) is a dry species outside the 1:NDRY set
c and has no LW table of its own; use the dust class (NA=6) as placeholder,
c which is what the whole DD2/DS2 mode got via CORE_CLASS before mixing.
             if (dry_Vf_LEV(l,n,8) > 0.d0) then
                CALL GET_LW(l,6,0,Reff_LEV(l,n),AMP_TAB_S,Vf)
                AMP_TAB(:) = AMP_TAB(:) + dry_Vf_LEV(l,n,8) * AMP_TAB_S(:)
             endif
             TAB(l,:) = TAB(l,:) + (AMP_TAB(:) *  TTAUSV(l,n) * FTTOPX(n))
          ENDDO   ! modes
          ENDDO   ! level
     ...
     +                     + dry_Vf_LEV(l,n,s) * AMP_TAB_CLASS(:,SPC_TO_NA(s),n)
             ENDDO   ! dry species
c SAI: calcite (slot 8) with the dust LW class as placeholder, as above.
             AMP_TAB(:) = AMP_TAB(:)
     +                  + dry_Vf_LEV(l,n,8) * AMP_TAB_CLASS(:,6,n)
             TAB(l,:) = TAB(l,:) + (AMP_TAB(:) *  TTAUSV(l,n) * FTTOPX(n))
          ENDDO   ! modes
          ENDDO   ! level
      endif
...
c      DD2/DS2 "dust" tracer mass to particle volume (cf DENS_DUST=2.60D+03)
      ...
      COMPLEX*8, DIMENSION(6,14)     :: Ri !Including all biogenic BrC
#else
      COMPLEX*8, DIMENSION(6,12)     :: Ri !Including only isoprene BrC
#endif
#endif
#else
      COMPLEX*8, DIMENSION(6,10)     :: Ri !Including brC
#endif
#else
...
c#ifdef TRACERS_BrC
     ...
     &        (1.52000,   0.00540000)  ,(1.52775,   0.00000000),    !OC set to white
c#else
c     &        (1.46099,    0.0761930)  ,(1.48313,   0.00470000),    !Oc
c     &        (1.49719,   0.00470000)  ,(1.50805,   0.00480693),
c     &        (1.52000,   0.00540000)  ,(1.52775,    0.0144927),    !Model default: all organics are brown, enable this for model commit
c#endif
...
#ifdef TRACERS_BrC
     &       ,(1.46099,    0.0761930)  ,(1.48313,   0.00470000),    !BrC_w
     &        (1.49719,   0.00470000)  ,(1.50805,   0.00480693),
     &        (1.52000,   0.00540000)  ,(1.52775,   0.01650000),    !Moderately absorbing case

     &        (1.46099,    0.0761930)  ,(1.48313,   0.00470000),    !BrC_b
     &        (1.49719,   0.00470000)  ,(1.50805,   0.00480693),
     &        (1.52000,   0.00540000)  ,(1.52775,   0.02930000), 

     &        (1.46099,    0.0761930)  ,(1.48313,   0.00470000),    !BrC_t
     &        (1.49719,   0.00470000)  ,(1.50805,   0.00480693),
     &        (1.52000,   0.00540000)  ,(1.52775,   0.00266000) 
#ifdef TRACERS_BIOGENIC_BrC
#ifdef TRACERS_AEROSOLS_SOA
#ifdef TRACERS_TERP
     &       ,(1.46099,    0.0761930)  ,(1.48313,   0.00470000),    !apinp1a                        
     &        (1.49719,   0.00470000)  ,(1.50805,   0.00480693),                                    
     &        (1.52000,   0.00540000)  ,(1.52775,   0.000900501),                                   
                                                                                                    
     &        (1.46099,    0.0761930)  ,(1.48313,   0.00470000),    !apinp2a                        
     &        (1.49719,   0.00470000)  ,(1.50805,   0.00480693),                                    
     &        (1.52000,   0.00540000)  ,(1.52775,   0.00049132)                                     
#endif                                                                                              
     &       ,(1.46099,    0.0761930)  ,(1.48313,   0.00470000),    !isopp1a                        
     &        (1.49719,   0.00470000)  ,(1.50805,   0.00480693),                                    
     &        (1.52000,   0.00540000)  ,(1.52775,   0.00227988),                                    
                                                                                                    
     &        (1.46099,    0.0761930)  ,(1.48313,   0.00470000),    !isopp2a                        
     &        (1.49719,   0.00470000)  ,(1.50805,   0.00480693),                                    
     &        (1.52000,   0.00540000)  ,(1.52775,   0.00226378)
#endif
     &        /
#else
     &        /
...
#ifdef TRACERS_AEROSOLS_SOA
#ifdef TRACERS_TERP
c                  SO4,  Seasalt,  NO3,    OC,     BCI,    BCB,   dust,    BrC_w,  BrC_b,   BrC_t   
      real*8, parameter, dimension(14)   ::
     &     hygro=(/0.53d0, 1.12d0, 0.54d0, 0.15d0, 0.01d0, 0.01d0, 0.00d0, 0.15d0, 0.15d0, 0.15d0   
c            apinp1a,apinp2a,isopp1a,isopp2a
     &      ,0.15d0, 0.15d0, 0.15d0, 0.15d0/)
      real*8, parameter, dimension(14)  :: density_aer=(/1.760,2.165,1.725,1.500,1.300,1.300,1.200  
c            BrC_w,BrC_b,BrC_t,apinp1a,apinp2a,isopp1a,isopp2a
     &      ,1.500,1.500,1.500,1.500,1.500,1.500,1.500/)
#else
c                  SO4,  Seasalt,  NO3,    OC,     BCI,    BCB,   dust,    BrC_w,  BrC_b,   BrC_t   
      real*8, parameter, dimension(12)   ::
     &     hygro=(/0.53d0, 1.12d0, 0.54d0, 0.15d0, 0.01d0, 0.01d0, 0.00d0, 0.15d0, 0.15d0, 0.15d0   
c            isopp1a,isopp2a
     &      ,0.15d0, 0.15d0/)
      real*8, parameter, dimension(12)  :: density_aer=(/1.760,2.165,1.725,1.500,1.300,1.300,1.200  
c            BrC_w,BrC_b,BrC_t,isopp1a,isopp2a
...
c                  SO4,  Seasalt,  NO3,    OC,     BCI,    BCB,   dust,    BrC_w,  BrC_b,   BrC_t
     ...
     &     hygro=(/0.53d0, 1.12d0, 0.54d0, 0.15d0, 0.01d0, 0.01d0, 0.00d0, 0.15d0, 0.15d0, 0.15d0/)                                     
      real*8, parameter, dimension(10)  :: density_aer=(/1.760,2.165,1.725,1.500,1.300,1.300,1.200
c            BrC_w,BrC_b,BrC_t
     &      ,1.500,1.500,1.500/)
#endif
#else
c                  SO4,  Seasalt,  NO3,    OC,     BCI,    BCB,   dust 
     ...
     &     hygro=(/0.53d0, 1.12d0, 0.54d0, 0.15d0, 0.01d0, 0.01d0, 0.00d0/)
      real*8, parameter, dimension(7)  :: density_aer=(/1.760,2.165,1.725,1.500,1.300,1.300,1.200/) 
#endif
#ifndef OMA_TRAMPRAD
#ifdef USE_OFFLINE_AEROSOLS
...
!          [ - ]                        [trm units: #/m2/layer]      
       ...
       DO n=1,nmodes  ![#/m2]         pi/4     [m2]
        ...
        ! NO3   
        VMass(n,6) = VMass(n,1) /(Sum(VMass(:,1)) + TINYNUMER) * NO3/ 1720.
