...
#ifdef TRACERS_BrC
      use aerosol_sources, only: BrC_wemifactBB
#endif  /* TRACERS_BrC */
...
!@var so4_fraction mole fraction of so2 to be emitted as so4
     ...
     &                 (1.d0-akk_fraction)
        case ('M_AKK_SU')
          get_src_fact=so4_fraction*tr_mm(n)/tr_mm(get_src_index(n))*
     &                 akk_fraction
        case ('OCII', 'OCIA', 'M_OCC_OC', 'M_BOC_OC', 'AOCOB_01')
          get_src_fact=1.d0
          if (.not.OA_not_OC_local) get_src_fact=get_src_fact*om2oc(n)
        case ('OCB')
#ifdef TRACERS_BrC          
          get_src_fact=(1.d0-BrC_wemifactBB)*1.0d0 !Option to scale up organic BB emissions here
          if (.not.OA_not_OC_local) get_src_fact=get_src_fact*om2oc(n)
        case ('BrC_w')
          get_src_fact=BrC_wemifactBB*1.0d0 !Option to scale up organic BB emissions here
          if (.not.OA_not_OC_local) get_src_fact=get_src_fact*om2oc(n)
#else
          get_src_fact=1.d0
          if (.not.OA_not_OC_local) get_src_fact=get_src_fact*om2oc(n)
...
!@auth Gavin Schmidt
      ...
      use TRACER_COM, only: nbiomass, nPyrE
...
!-----
        ...
        if(do_fire(n_src))then
          itcon_3Dsrc(nPyrE,n)=tr_con_diag('pyrE src',T,T)
        endif
     ...
     *           ,'Terpenes','Acetone')
     ...
     &          'vbsAp3', 'vbsAp4', 'vbsAp5', 'vbsAp6')
          ...
          case ('BCIA', 'OCIA')
            itcon_3Dsrc(nChemistry,n)=tr_con_diag('Aging source',T,T)
          case ('BrC_b')
            itcon_3Dsrc(nChemLoss,n)=tr_con_diag('Bleaching loss',T,T)
            itcon_3Dsrc(nChemProd,n)=tr_con_diag('Browning source',T,T)
          end select
...
! pyrE emissions
      if (do_fire(n_src)) then
        k = k + 1
        jls_3Dsource(nPyrE,n) = k
        sname_jls(k) = trim(trname(n))//'_pyrE_src'
        lname_jls(k) = trim(trname(n))//' pyrE source'
        jls_ltop(k) = LM
        jls_power(k) = -2
        units_jls(k) = unit_string(jls_power(k),tend_units)
      endif
...
!         Bleaching loss          
          ...
          jls_3Dsource(nChemloss,n) = k
          sname_jls(k) = 'chem_sink_of_'//trim(trname(n))
          lname_jls(k) = 'bleaching loss of BrC_b'
          jls_ltop(k) = LM
          jls_power(k) = -1
          units_jls(k) = unit_string(jls_power(k),tend_units)
!         Browning source
          k = k + 1
          jls_3Dsource(nChemprod,n) = k
          sname_jls(k) = 'chem_source_of_'//trim(trname(n))
          lname_jls(k) = 'browning source of BrC_b'
          jls_ltop(k) = LM
          jls_power(k) = -1
          units_jls(k) = unit_string(jls_power(k),tend_units)
        end select
...
!@auth Gavin Schmidt
      ...
      use TRACER_COM, only: nOther, nOverwrite, nVolcanic, nChemloss
      use TRACER_COM, only: nChemprod, nThermo, nMicrophys, nPyrE
      use TRACER_COM, only: ntsurfsrc, tracers, aqchem_list
      use OldTracer_mod, only: do_aircraft
      use TRACER_COM, only: nRocket
      use OldTracer_mod, only: do_rocket
...
! pyrE emissions
      if (do_fire(n_src)) then
        ijts_3Dsource(nPyrE,n)=
     *    ijts_diag(trim(trname(n))//'_pyrE_src',
     *              trim(trname(n))//' pyrE source',
     *              'kg m-2 s-1', power=-12,
     *              scalediv=dtsrc)
      endif
...
c#endif
     ...
     *              scalediv=dtsrc)
     ...
     &      'BrC_w', 'BrC_b', 'BrC_t'
#ifdef TRACERS_BIOGENIC_BrC
     &      ,'isopp2a','apinp1a','apinp2a')
#else
     &      )
#endif
        call set_diag_aod(n)
     ...
     *      ijts_diag(trim(trname(n))//'_chemloss',
     *                trim(trname(n))//' bleaching loss',
     *                'kg m-2 s-1', power=-12,
     *                scalediv=dtsrc)
...
C**** Note this routine must always exist (but can be a dummy routine)
    ...
    (defined TRACERS_AMP) || (defined TRACERS_TOMAS) 
      ...
      use trdust_mod, only : dust_bin1_stream, dust_bin2_stream,
     &     dust_bin3_stream, dust_bin4_stream
#endif
#ifdef PRESC_BB_INJ
      USE timestream_mod, only: init_stream,read_stream
    ...
    (defined TRACERS_TOMAS)
      ...
      use OldTracer_mod, only: nBBsources,vol2mass, do_fire
...
c     seconds per day, since the subroutine for dust emission, through
c     which the fluxes go, expects the fluxes to be in kg/m^2/s.
...
! Apply changes to q. do not call update_qvmom, since the concentration
! always increases. Do that after the tracers loop, not before or during.
      if (direct_inject_num > 0) then
        if (sum(direct_inject_H2O) > 0.d0) then
          call update_qv(qv(:)+H2O_src_3d(i,j,:)*dtsrc*hour_fact)
        endif ! else nothing, no direct injection of water
      endif ! else nothing, no direct injection of anything
...
!@var src_index source index for the current tracer
!@var src_fact source factor for the current tracer
      integer :: src_index,get_src_index,bb_i,bb_e,pyre_i
      real*8 :: src_fact, hour_fact
...
C**** 3D biomass source
     ...
     &            sum(sfc_src(i,j,src_index,bb_i:bb_e))
          ...
          ! Add contribution to 3D pyrE diagnostic
          if(do_fire(src_index)) then ! pyrE sources always last
            pyre_i=ntsurfsrc(src_index)+nBBsources(src_index)+1 ! index of first pyrE source
            blsrc_pyrE=tune_BBsources*src_fact*
     &            sfc_src(i,j,src_index,pyre_i)
          endif
...
! Time-varying prescribed injection heights from file
     ...
     &        (sum(sfc_src(i,j,src_index,bb_i:bb_e)) .ne. 0.d0)) then
        ...
        if (do_fire(src_index)) then
          do l=1,LM ! fire_src_3d_fact is zero outside plume bottom and top
            tr3Dsource(l,nPyrE,n)=blsrc_pyrE*fire_src_3d_fact(l,i,j)
          end do
          if ((sum(fire_src_3d_fact(:,i,j)) .eq. 0.d0) .and.
     &        (sfc_src(i,j,src_index,pyre_i) .ne. 0.d0)) then
            tr3Dsource(1,nPyrE,n)=blsrc_pyrE
          endif
        endif
...
! Standard plume distribution through boundary layer
          ...
          if (do_fire(src_index)) then
            blsrc_pyrE=blsrc_pyrE/sum(MA(1:blay))
            do l=1,blay
              tr3Dsource(l,nPyrE,n)=blsrc_pyrE*MA(l)
            end do
          endif
#endif  /* PRESC_BB_INJ */
#ifdef DIURN_BB
...
#endif
#ifndef TRACERS_TOMAS
        if (nBBsources(src_index) > 0)
     &    call apply_tracer_3Dsource(i,j,nBiomass,n)
        if (do_fire(src_index)) call apply_tracer_3Dsource(i,j,nPyrE,n)
#else
...
! nbins ones are all water tracers.
     ...
     &                      n_apinp2a, n_NH4, n_NO3p
      ...
      use PBLCOM, only: pblptop
      use atmcol_com, only: pl,ple ! layer pressure (mb), pressure layer at top of box
#endif  /* TRACERS_BrC */
      use TimeConstants_mod, only: SECONDS_PER_DAY
...
#ifdef TRACERS_BrC
      real*8 :: OH_conc, O3_conc, NO3_conc !oxidant concentrations
      real*8 :: OH_w2b_efold, N_w2b_efold, OH_b2t_efold, O3_b2t_efold !efold times for browning/bleaching
      real*8 :: w2b_efold,b2t_efold
      real*8 :: pblp !PBL pressure
      integer :: l
      real*8 :: fw2b,fb2t !exponential decay factors
#endif /* TRACERS_BrC */
...
#ifdef TRACERS_BrC
        case ('BrC_w')
        do l=1,lm !After this line, I think I can put an if-def statement to move on to the next layer if this is above the PBL: if (z(l).gt.pblht(l)):
          pblp=pblptop(i,j) !Get current PBL top height
          if (ple(l).gt.pblp) then !For layers containing the pbl top and all layers above: skip the chemistry
            continue
          endif
          call get_oxidants(i,j,l)
          if (oxid%OH==0.d0) then
            OH_conc=1.d-10
          else
            OH_conc=oxid%OH
          endif
          if (oxid%NO3==0.d0) then
            NO3_conc=1.d-10
          else
            NO3_conc=oxid%NO3
          endif
          OH_w2b_efold=1.d0/(10.0d-1*(1.9d-11)*OH_conc)
          N_w2b_efold=1.d0/(10.0d-1*(7.90008d-14)*NO3_conc)
          w2b_efold=1.d0/((1/OH_w2b_efold)+(1/N_w2b_efold))
!          w2b_efold=10.804d0*3600.0d0 !Option for simulation of fixed lifetimes

!         Apply two chemical reactions in parallel:
          fw2b=(1.d0-exp(-dtsrc/(w2b_efold)))/dtsrc
          tr3Dsource(l,nChemistry,n)=-fw2b*trm_col(l,n) !loss from BrC_w
          tr3Dsource(l,nChemprod,n_BrC_b)=-tr3Dsource(l,nChemistry,
     &                                   n)*(tr_mm(n_BrC_b)/
     &                                   tr_mm(n)) !gain of BrC_b
        enddo
        call apply_tracer_3Dsource(i,j,nChemistry,n)
        call apply_tracer_3Dsource(i,j,nChemprod,n_BrC_b)

        case ('BrC_b')
        do l=1,lm
          pblp=pblptop(i,j) !Get current PBL height
          if (ple(l).gt.pblp) then !For layers containing the pbl top and all layers above: skip the chemistry
            continue
          endif
          call get_oxidants(i,j,l)
          ...
          if (oxid%O3==0.d0) then
            O3_conc=1.d-10
          else
            O3_conc=oxid%O3
          endif
          OH_b2t_efold=1.d0/(10.0d-1*(4.4d-11)*OH_conc)
          O3_b2t_efold=1.d0/(10.0d-1*(9.14123d-16)*O3_conc)
          b2t_efold=1.d0/((1/OH_b2t_efold)+(1/O3_b2t_efold))
!          b2t_efold=0.294d0*3600.0d0 !Option for simulation of fixed lifetimes

!         Apply two chemical reactions in parallel
          fb2t=(1.d0-exp(-dtsrc/(b2t_efold)))/dtsrc
          tr3Dsource(l,nChemloss,n)=-fb2t*trm_col(l,n) !Loss of BrC_b
          tr3Dsource(l,nChemistry,n_BrC_t)=-tr3Dsource(l,nChemloss,
     &                                   n)*(tr_mm(n_BrC_t)/
     &                                   tr_mm(n)) !Gain of BrC_t
        enddo
        call apply_tracer_3Dsource(i,j,nChemloss,n)
        call apply_tracer_3Dsource(i,j,nChemistry,n_BrC_t)
