...
!@var LDATE length of date string (7 or 12)
      ...
      if(JMONL>INT_MONTHS_PER_YEAR .and. months<=INT_MONTHS_PER_YEAR)
     *  yr1 = yr1+1
      JMONL = 1 + mod(JMONL-1,INT_MONTHS_PER_YEAR)
...
!!    end if ! only needed for off-line applications

      write(aDATE(4:7),'(a4)') i5toc4(yr1)
      if(years.gt.1) then ! months is a multiple of INT_MONTHS_PER_YEAR
        yr2=yr1+years-1
        write(aDATE(8:12),'(a1,a4)') '-',i5toc4(yr2)
        aDATE(1:3) = 'ANN'
      end if

!**** The remainder is only meaningful if INT_MONTHS_PER_YEAR=12
      if(INT_MONTHS_PER_YEAR.ne.12) return
      if(months.gt.12.and.mod(months,12)>0) aDATE(1:1)='x' ! should not happen
      if(months.le.1 .or. aDATE(1:1)=='x') return
